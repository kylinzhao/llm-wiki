...
9. 如果收口检查失败或仍有可安全修复的问题，优先继续修复或建议继续 `llm-wiki update`，不要建议 ship。
10. 如果收口检查通过且没有阻塞项，在 `建议下一步` 中提醒用户可以选择 `$llm-wiki-ship` / `llm-wiki ship` 进入发布、提交或推送收口；不要自动 ship，除非用户明确要求。
11. 最后输出 `建议下一步`。
