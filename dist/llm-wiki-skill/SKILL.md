...
- `llm-wiki pull`：只拉最新、不做精修的轻量预热命令。同步 KB git、`raw/`（按 `upstream/wiki-sources.json`）和 `raw-code/`（按 `upstream/code-sources.json`）证据缓存，报告上次更新时间与上次精修时间，并根据 `now - last_update_time` 是否超过 1 天给出 `直接 query` 或 `建议 update` 的结论；不会修改 `wiki/`、`staging/`、`graph/`、`index/` 或 `tools/` 产物，也不进入 shared publish。
...
- `llm-wiki update-skill`：显式更新已安装的 llm-wiki skill bundle 本体，并默认同时升级上游 `prd-review-max`；不要混入普通 KB 内容更新，除非用户明确要求。
