...
| `llm-wiki code-trace` | User wants targeted raw-code scan rebuild, code candidate diagnostics, traceability units, or code-side AI traceability refinement | Code trace diagnostics, rebuild artifacts, and AI-refined unit-level traceability |
...
| `llm-wiki pull` | User wants a no-refresh warm-up: sync KB git + `raw/` + `raw-code/` evidence caches and report last update/refinement times, without touching `wiki/` / `staging/` / `graph/` / `tools/` | Freshness report (KB git HEAD, `raw/` + `raw-code/` sync status, last update time, last refinement time) and a `≤1d 直接 query / >1d 建议 update` verdict |
