...
DEFAULT_GIT_URL = "https://git.guazi-corp.com/c2b-fe/llm-wiki-new.git"
...
def gitlab_auth_help(git_url: str) -> str:
    return (
        ...
        "Then retry after configuring Git credentials, or pass --source /path/to/llm-wiki-new-new-skill "
        "to install from a local checkout."
    )


def default_bundle_root() -> Path | None:
    # Source checkout layout: <bundle>/skills/llm-wiki-new/scripts/this_file.py
    ...
    for base in (cwd, *cwd.parents):
        for candidate in (base, base / "llm-wiki-new-skill"):
            if is_bundle_root(candidate):
                return candidate
    return None
...
def default_cache_dir(cache_dir: str | None) -> Path:
    ...
    return Path("~/.cache/llm-wiki-new-new-skill").expanduser().resolve()
...
def resolve_bundle_root(source: str | None, git_url: str | None, cache_dir: str | None, no_download: bool) -> Path:
    if source:
        ...
        if root is None:
            if no_download:
                raise SystemExit(
                    "Could not infer the llm-wiki-new skill bundle checkout from this installed skill. "
                    "Pass --source /path/to/llm-wiki-new-new-skill, or allow GitLab download."
                )
    ...
    if not is_bundle_root(root):
        ...
        raise SystemExit(f"Missing skills/llm-wiki-new in bundle source: {root}")
    return root
...
def main() -> int:
    ...
    parser.add_argument(
        "--source",
        help="Local llm-wiki-new-skill bundle checkout. Overrides local discovery and GitLab download.",
    )
    parser.add_argument(
        "--git-url",
    ...
    parser.add_argument(
        "--cache-dir",
        help="Directory for the downloaded GitLab checkout. Defaults to $LLM_WIKI_SKILL_CACHE_DIR or ~/.cache/llm-wiki-new-new-skill.",
    )
    parser.add_argument(
        "--client",
        default="auto",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults to $LLM_WIKI_SKILL_BACKUP_DIR or ~/.llm-wiki-new-skill-backups."
        ),
    )
    ...
    if code == 0:
        ...
        print(
            "Run `llm-wiki-new maintain-all` / `$llm-wiki-new-maintain-all` to preview batch backfill/update for registered KBs."
        )
    return code
