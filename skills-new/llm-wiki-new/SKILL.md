...
- `llm-wiki-new maintain-all`：维护本机 `~/.llm-wiki/projects.json` KB registry；默认 dry-run，可用 `--discover` 补录历史 KB、`--list` 查看、`--prune-missing` 清理不存在路径，只有用户明确要求 `--apply` 时才对已注册 KB 批量执行完整 backfill/update 维护。
