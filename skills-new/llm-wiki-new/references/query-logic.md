...
- **业务知识 / 产品规则 / 需求口径 / 术语解释**：只回答业务与需求结论，优先使用 `BUSINESS_CONTEXT.md`、`overview`、按问题类型命中的专项目录层、`concepts`、`entities`、`sources`。不要主动展开代码实现细节；如需说明实现相关性，只用一句话标注“本回答未核验代码实现”或“如需代码落地证据请使用 `llm-wiki-new query-plus`”。
...
6. 如用户需要实现落地验证，建议改用 `llm-wiki-new query-plus`
