...
`$LLM_WIKI_NEW_SKILL_ROOT` 为 llm-wiki-new skill 包根目录（见主 `SKILL.md`「Skill 包路径」）。
...
如果存在 `raw-code/`，标准文档构建之后还要进入代码 wiki 阶段。`raw-code/*` 只支持由 `llm-wiki-new add-code` 创建的 engine-managed git checkout；旧的复制、软链或外部路径形态属于迁移对象，不再是合法终态。
...
- 当项目已经配置启用 feed 时，`llm-wiki-new update` / `tools/update_wiki.py` 应默认先运行这一阶段刷新 `raw/`，而不是要求用户手动补 `--raw-sync-command`
...
- 要求每个 codebase 都是 `llm-wiki-new add-code` 创建的受管 git checkout
...
- health pass。
- graph rebuilt。
- 图片证据状态：如果 `raw/` 存在图片资产但尚未完成高价值图片证据补充，明确记录为阶段 H pending，并在建议下一步中推荐 `llm-wiki-new image`。
