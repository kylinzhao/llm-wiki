...
代码 wiki 是 `llm-wiki-new` 的可选一等证据层。

当项目根目录存在 `raw-code/` 时，`llm-wiki-new` 应将源码仓库纳入构建、审查和查询协议。代码 wiki 的目标不是替代需求 wiki，而是回答：
...
`raw-code/*` 的每个一级目录是独立源码库，称为 `codebase_id`。唯一受支持的形态是 `llm-wiki-new add-code` 创建的 engine-managed git checkout。
