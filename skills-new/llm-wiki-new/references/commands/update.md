...
- `upstream/wiki-sources.json` restores `raw/`. `upstream/code-sources.json` restores `raw-code/` as engine-managed git checkouts created by `llm-wiki-new add-code`.
...
5. Repair project agent query-routing rules when the standard template tooling is available:
   ...
   - This is the automatic migration path for existing KB projects; do not ask the user to run `--agent-rules-only` manually during normal `llm-wiki-new update`.
   ...
   - `llm-wiki-new doctor` should only report missing agent rules; it should not modify files.
...
7. Run the deterministic project update command when available, such as `uv run python tools/update_wiki.py`.
   ...
   - when `raw-code/` codebases are connected through `llm-wiki-new add-code`, prefer an update command path that includes the code refresh automatically by auto-running `git pull --ff-only` per clean managed codebase inside `tools/update_wiki.py`
...
- End by asking the user to run `llm-wiki-new update` again for low-risk pending/stale/source/traceability work that can be completed now.
...
- Do not recommend `llm-wiki-new update` as the next step when the current `llm-wiki-new update` can safely finish the remaining source refinement, capability, traceability, health, or graph work. Finish it in the current command.
...
- If affected source pages remain stale and affected code traceability also needs refresh but a hard blocker prevents completion, report the blocker and checkpoint, then recommend one combined continuation: `llm-wiki-new update` to resume the integrated source refinement plus traceability refresh.
- Traceability-only and source-only refinements still stay under `llm-wiki-new update`; do not route to separate commands.
- When validation fails, recommend the smallest safe continuation or fix, phrased as a command the user can run (`llm-wiki-new update`, `llm-wiki-new doctor`, or `llm-wiki-new image`) rather than a script chain.
