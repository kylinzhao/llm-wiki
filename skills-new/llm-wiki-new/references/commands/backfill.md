...
5. If `refinement_absorption_required=true`, continue directly with `llm-wiki-new update` semantics over `refinement_scope`:
...
6. If no deterministic evidence changed, run or recommend `llm-wiki-new doctor` for a read-only confirmation.
