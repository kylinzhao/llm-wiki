...
`llm-wiki-new` 是一个全局可调用的 Codex skill，用来处理一套文件型 LLM Wiki 的完整生命周期：
...
对于已经建好的 wiki，查询也统一通过 `llm-wiki-new` 完成，不再维护单独的 query-only skill。
...
- `用 $llm-wiki-new fast 从 raw/ 和 BUSINESS_CONTEXT.md 初始化新项目，一次性跑完标准首轮。`
- `用 $llm-wiki-new doctor 看看整个站点健康度、缺口和下一步建议。`
- `用 $llm-wiki-new update 响应这次文档和代码变更，只更新受影响页面；如果项目已配置 RSS/feed 或接入了 raw-code codebase，就先默认刷新它们；结束前自动检查收口。`
- `用 $llm-wiki-new-backfill 补齐老知识库的历史 draw.io、Jira/Cjira、source metadata 等证据，并继续精修吸收。`
- `用 $llm-wiki-new-maintain-all 发现并维护本机已注册的多个 KB，先 dry-run 再确认 apply。`
- `用 $llm-wiki-new update-skill 更新本机安装的 llm-wiki-new skill bundle。`
- `用 $llm-wiki-new review-requirement 帮我 review 这个 Cwiki 需求，并输出评论稿。`
- `用 $llm-wiki-new update 补某个业务能力的需求到代码追踪矩阵。`

### 可发现的二级 skill

本 bundle 还提供了一组 `$llm-wiki-new-*` 短入口。它们等价于主入口的二级命令，用来减少提示词长度、提高 Codex skill 列表里的可发现性。

| 二级 skill | 等价命令 | 适合什么时候用 |
| --- | --- | --- |
| `$llm-wiki-new-fast` | `llm-wiki-new fast` | 新项目标准首轮，一口气完成构建、精修、验证和收口。 |
| `$llm-wiki-new-init` | `llm-wiki-new init` | 新项目分阶段初始化。 |
| `$llm-wiki-new-doctor` | `llm-wiki-new doctor` | 只读诊断项目健康度、质量问题、缺口和下一步；集合原 audit 能力。 |
| `$llm-wiki-new-update` | `llm-wiki-new update` | 输入、wiki 或源码变化后的影响范围更新；也负责续跑、精修、代码 wiki、traceability 和收口检查；会自动维护 `AGENTS.md` 查询路由规则。 |
| `$llm-wiki-new-backfill` | `llm-wiki-new backfill` | 存量 KB 历史证据补全；重新扫描历史 raw/wiki/staging，补齐新版确定性派生能力，并继续进入 source/G+ 精修吸收。 |
| `$llm-wiki-new-maintain-all` | `llm-wiki-new maintain-all` | 维护本机已注册 KB registry；默认 dry-run，可发现、列出、清理 missing 项，并在确认后批量执行完整 backfill/update 维护。 |
| `$llm-wiki-new-update-skill` | `llm-wiki-new update-skill` | 显式更新本机安装的 llm-wiki-new skill bundle；不更新当前 KB 内容。 |
| `$llm-wiki-new-add-wiki` | `llm-wiki-new add-wiki` | 接入新的文档/wiki 来源到 `raw/`。 |
| `$llm-wiki-new-add-code` | `llm-wiki-new add-code` | 接入新的源码库到 `raw-code/`，并构建代码 wiki、能力页和必要 traceability。 |
| `$llm-wiki-new-query` | `llm-wiki-new query` | 按意图回答业务或实现问题；业务知识默认不展开大量代码证据。 |
| `$llm-wiki-new-query-plus` | `llm-wiki-new query-plus` | 同时回答业务/需求口径与代码实现证据，适合需要更详尽联动分析的问题。 |
...
- 由 `llm-wiki-new` 将图谱结果转成可审计 Markdown
...
`graphify` 不替代 `llm-wiki-new` 的业务语义协议，也不替代由 wikilink 生成的项目 `graph/`。`docs/wiki` 与 graphify 都不能单独证明 `strong` traceability；强证据仍需要需求锚点和直接源码锚点。
...
标准命令（`$LLM_WIKI_NEW_SKILL_ROOT` 为 llm-wiki-new skill 包根目录，见主 `SKILL.md`「Skill 包路径」）：

```text
python3 "$LLM_WIKI_NEW_SKILL_ROOT/scripts/install_project_template.py" --project "$PWD"
...
已构建的老 KB 只要通过新版 `$llm-wiki-new update` 进入维护流程，agent 应先刷新 engine-owned 项目工具并合并 agent 查询路由规则；此后每次新版 `tools/update_wiki.py` 都会自动检查。若只想补 `AGENTS.md`，不要使用 `--force` 覆盖模板文件；在 KB 根目录运行：

```text
python3 "$LLM_WIKI_NEW_SKILL_ROOT/scripts/install_project_template.py" --project "$PWD" --agent-rules-only
```
...
更新来源优先使用本地 `llm-wiki-new-skill` bundle checkout 的 git upstream。若无法从安装目录、环境变量或当前工作目录推断本地 checkout，updater 会从公司 GitLab 默认地址 `https://git.guazi-corp.com/c2b-fe/llm-wiki-new.git` 下载到 `~/.cache/llm-wiki-new-new-skill/llm-wiki-new`，再从该 cache 安装。可用 `--git-url` / `LLM_WIKI_SKILL_GIT_URL` 覆盖下载地址，用 `--cache-dir` / `LLM_WIKI_SKILL_CACHE_DIR` 覆盖 cache 目录；GitHub remote 只作为额外远端，除非显式指定，不默认使用。若下载私有 GitLab 仓库时缺少凭据，Personal Access Token 创建地址是 `https://git.guazi-corp.com/profile/personal_access_tokens`，所需 scope 为 `read_repository`。
...
使用 `llm-wiki-new` 及其相关短入口时，面向用户的回答、诊断、审查报告和最终总结默认使用中文；由 agent 生成或改写的 `wiki/`、`docs/`、`staging/` Markdown 知识文档也默认使用中文。代码标识符、命令、路径、API 名称、配置键、英文专有名词和原始证据引用可保留原文，并用中文解释。
...
如果核心页面依赖流程图、状态截图、数据表、验收图、账户/费用/保证金/风控/权限类图片，项目只能称为“文本层健康”；仍应把 `llm-wiki-new image` 作为高价值证据补强的建议下一步。

例外：`llm-wiki-new review-requirement` 进行需求评审时，图片、截图、图表和 zip 原型都属于需求证据；必须分析图片内容，并把 zip 优先当作 HTML 原型检查。
...
```text
用 $llm-wiki-new fast 从 raw/ 和 BUSINESS_CONTEXT.md 初始化新项目，并按标准顺序一次性完成首轮构建、精修、代码 wiki、验收、health 和 graph。
```

```text
用 $llm-wiki-new doctor 诊断整个 LLM Wiki 站点状态，告诉我哪里健康、哪里缺口最大、下一步建议怎么做。
```

```text
用 $llm-wiki-new update 响应这次文档或代码变更。先判断影响范围；如果项目已配置 raw 或接入了 raw-code codebase，就先默认刷新它们，再只更新受影响的 source、concept/entity、code/capability/traceability 页面，最后跑 health 和 graph。
```

```text
用 $llm-wiki-new trace 补电话直连链路的需求到代码追踪矩阵，标注 strong/partial/inferred/external/missing 并补关键代码锚点。
```

```text
用 $llm-wiki-new 继续维护这个项目。先读 BUSINESS_CONTEXT.md 和当前状态，从未完成阶段续跑，不要重建已完成内容。
```

```text
用 $llm-wiki-new 构建需求+代码联合 wiki。raw 是需求证据，raw-code 是代码证据，按现有协议并发推进并输出验收结果。
```

```text
用 $llm-wiki-new 查询这个问题。先读 BUSINESS_CONTEXT.md，说明查询类型、检索路径、结论、支撑页面和未决点。
```

### 新项目 0-1

```text
Use $llm-wiki-new to bootstrap a new LLM Wiki project from raw/ and BUSINESS_CONTEXT.md. Explain the phases first, initialize the structure, run deterministic build steps, then complete the first-round full-corpus summary and AI-native refinement in the same run if feasible. Layered execution is allowed, but do not stop after only generating the skeleton.
```

### 已有项目查询

```text
Use $llm-wiki-new against this project. Read BUSINESS_CONTEXT.md first, state the query type and retrieval path, then answer with supporting wiki pages and unresolved points. If the question is business-only, do not include detailed code evidence unless necessary.
```

### 质量审查

```text
Use $llm-wiki-new doctor to review this wiki project. Check entry usability, semantic consistency, retrieval usefulness, layered-page quality, and concept/entity conflicts. Put findings first with file paths and a final usability verdict.
```

### 需求 + 代码联合 wiki

```text
Use $llm-wiki-new to build a joint business-and-code LLM Wiki. Read BUSINESS_CONTEXT.md first, treat raw/ as immutable business evidence, treat raw-code/* as separate codebases, use graphify when available for code graph extraction, then create wiki/code/codebases and wiki/code/capabilities pages that link code facts back to concepts, entities, and source documents.
```
