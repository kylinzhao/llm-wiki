...
def build_report(project: Path) -> dict[str, object]:
    ...
    if (raw_image_count or raw_diagram_count) and image_note_count == 0 and image_evidence_status not in {"complete", "not_applicable", "skipped_by_user"}:
        image_evidence_gaps.append(
            "raw/ contains image or draw.io diagram assets but no staging/image-notes/ were found; after text/G+ completion, review high-value visual evidence with `llm-wiki-new image`."
        )
    if drawio_status["missing_evidence_count"]:
        image_evidence_gaps.append(
            f"raw/ contains {drawio_status['missing_evidence_count']} draw.io diagram(s) without generated .drawio.md text evidence; run `llm-wiki-new update` to repair draw.io evidence."
        )
    ...
    if raw_gap_message:
        recommended_actions.append(
            "Restore raw/ (git submodule, sparse checkout, LFS, or internal sync), then run `llm-wiki-new update`."
        )
    if raw_code_gap_message:
        recommended_actions.append(
            "Restore raw-code/<codebase_id>/, then run `llm-wiki-new update`."
        )
    if image_evidence_gaps:
        recommended_actions.append(
            "Run `llm-wiki-new image` for selective high-value image evidence after confirming the text layer is complete; do not batch-analyze low-value screenshots by default."
        )
    if drawio_status["missing_evidence_count"]:
        recommended_actions.insert(0, "Run `llm-wiki-new update` to generate missing draw.io Mermaid text evidence before semantic refinement.")
    if (
        drawio_promotion["not_promoted_count"]
        and not drawio_status["missing_evidence_count"]
    ):
        recommended_actions.append(
            f"{drawio_promotion['not_promoted_count']} draw.io source page(s) have not been promoted to concept/overview pages; run `llm-wiki-new update` to absorb drawio flow knowledge into wiki concepts."
        )
    if not business_context["has_valid_business_context"]:
        recommended_actions.insert(
            0,
            "Complete BUSINESS_CONTEXT.md with the project business baseline, canonical entities, rules, and evidence priority before running `llm-wiki-new init`, `llm-wiki-new fast`, or `llm-wiki-new update`.",
        )
    ...
    if cjira_registry["stale_status_pages"]:
        if local_jira_auth_configured():
            recommended_actions.append("Refresh active cjira statuses with `llm-wiki-new update`.")
        else:
            recommended_actions.append(
                "Refresh active cjira statuses with `llm-wiki-new update`; if Jira auth is missing, configure local SSO/Jira auth first."
            )
