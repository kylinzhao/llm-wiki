...
def write_success_report(
    ...
    if gplus_quality["findings"]:
        ...
        lines.extend(
            [
                "",
                "Next action: run the Codex-native G+ semantic expansion pass in `llm-wiki-new update`; do not rebuild `raw/` solely for these findings.",
            ]
        )
    else:
    ...
    if refinement_contract["status"] == "needs_refinement":
        lines.extend(
            [
                ...
                "",
                "Next action: the current `llm-wiki-new update` agent must run Codex-native source refinement for these pages before final closure; this is a P1 automatic update task, not a user follow-up.",
            ]
        )
    else:
            ...
            if is_permission_error(detail):
                ...
                if shared_mode:
                    message += (
                        ...
                        "如果只想在本机试跑，请显式使用 LLM_WIKI_UPDATE_MODE=local 或 $llm-wiki-new-update --local。"
                    )
