...
回答用户问题、输出诊断/审查报告，以及生成或改写 `wiki/`、`docs/`、`staging/` 中的 Markdown 知识文档时，默认使用中文。代码标识符、命令、路径、API 名称、配置键、英文专有名词和原始证据引用可保留原文，并用中文解释；只有用户明确要求其他语言时才切换。
...
def build_refinement_plan(
    ...
    return {
        ...
        "user_next_action": (
            "Continue `llm-wiki-new update` to complete source-grounded AI-native refinement, "
