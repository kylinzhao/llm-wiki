...
def write_markdown_report(project: Path, report: dict[str, object]) -> None:
    ...
    lines.extend(
        [
            ...
            "## Guidance",
            "",
            "Backfill has only repaired deterministic evidence. Continue with `llm-wiki-new update` semantics to absorb changed evidence into source summaries, concepts, entities, G+ layers, query acceptance, health, and graph.",
        ]
    )
