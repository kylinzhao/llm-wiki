---
name: llm-wiki-new-add-wiki
description: LLM Wiki 添加文档库入口。用于把另一个导出的 wiki、wiki URL、Markdown 文件夹、Confluence 导出、文档目录或业务/需求语料加入当前项目的 raw/ 原始证据层。（实验包 llm-wiki-new；与全局 llm-wiki 并行，验证通过后再合并）
---

# LLM Wiki Add Wiki

这是 `$llm-wiki-new add-wiki` 的短入口。
...
3. 将 `$llm-wiki-new-add-wiki` 后面的用户文本作为来源路径、wiki URL 和 `llm-wiki-new add-wiki` 参数。
