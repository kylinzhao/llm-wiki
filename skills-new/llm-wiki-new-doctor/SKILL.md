...
10. **Drawio 知识提升检查**：检查 health 报告中的 `drawio_promotion` 字段。当 `not_promoted_count > 0` 时，在主要问题中报告 drawio 流程知识未被 concept/overview 吸收，建议 `llm-wiki-new update` 做 agent-native 提升。
11. 最后输出 `建议下一步`。
