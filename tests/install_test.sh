...
test_force_replaces_existing_skill() {
  ...
  assert_no_path "$home/skills/llm-wiki/local.txt"
  assert_contains "$(sed -n '1p' "$ROOT_DIR/skills/llm-wiki/VERSION")" "$home/skills/llm-wiki/VERSION"
  assert_contains "$(sed -n '2p' "$ROOT_DIR/skills/llm-wiki/VERSION")" "$home/skills/llm-wiki/VERSION"
